...
export declare class Actions<S = {}, G extends BG<S> = BG<S>, M extends BM<S> = BM<S>, A = {}> {
    ...
    protected readonly dispatcher: Dispatcher<A>;
    protected readonly committer: Committer<M>;
}
export declare type CommitterMethod<Method> = Payload<Method> extends never ? never : Method extends (...args: infer Args) => infer R ? R extends void ? Method : (...args: Args) => void : never;
export declare type Committer<M extends Mutations<any>> = {
    [K in keyof M]: CommitterMethod<M[K]>;
};
export declare type DispatcherMethod<Method> = Payload<Method> extends never ? never : Method extends (...args: infer Args) => infer R ? R extends (void | Promise<any>) ? Method : (...args: Args) => Promise<R> : never;
